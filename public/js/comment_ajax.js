 $('#form').submit(function(event) {
    ...
    $.ajax({
      ...
      data['comments'].forEach(function(comment){ 
        ...
        let csrf = document.getElementById('csrf-token').content;
          // dataの中身をループをつかってresultsにどんどんいれていく
        commentData = 
            '<div class="modal fade" id="cyclecomment-comment'+comment.id+'" tabindex="-1" role="dialog" aria-labelledby="basicModal" aria-hidden="true">'+
            ...
            '<div class="modal fade" id="cyclecomment-delete'+comment.id+'" tabindex="-1" role="dialog" aria-labelledby="basicModal" aria-hidden="true">'+
                '<div class="modal-dialog">'+
                    '<div class="modal-content">'+
                        '<div class="modal-header">'+
                            '<h4>'+
            ...
            '<div class="modal fade" id="cyclecomment-comment-thread'+comment.id+'" tabindex="-1" role="dialog" aria-labelledby="basicModal" aria-hidden="true">'+
                '<div class="modal-dialog">'+
                    '<div class="modal-content">'+
        ...
        '<div class="form-row">'+
            '<div class="col-sm-8 offset-sm-2">'+
                '<div class="card cycle-comment cyclecomment-body-'+comment.id+'" style="min-height:150px; cursor:pointer; margin-top:10px;" onclick="postData('+comment.id+')">'+
                    '<div class="side" style="margin-left:8px; margin-top:8px;">'+
    ...
    $(document).on('click', '.comment-button', function(){ 
        ...
        $.ajax({
            ...
            // 成功したとき
            $("#cyclecomment-"+form_id).modal("hide");
                    ...
                    '<div class="modal-content">'+
                        ...
                        '<div class="modal-body">'+
                            '<form id="comment'+comment.id+'" method="POST" action="/ajax">'+
                                '<div class="form-group">'+
                                    '<input type="hidden" name="_token" value="'+csrf+'">'+
                                    '<input type="hidden" name="alert_id" value="'+comment.cycle_id+'">'+
                                    '<input type="hidden" name="parent_id" value="'+comment.id+'">'+
                                    '<textarea class="form-control" name="comment" style="font-size:1.3em;">'+
                                    '</textarea>'+
                                '</div>'+
